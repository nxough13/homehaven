...
// ----------- PRODUCTS CRUD -----------
// ---Code para sa MP2: NodeJS CRUD API (20pts)----
...
// ---Code para sa MP4: jQuery/datatables with multiple file uploads (20pts)----
...
router.post('/seller/products', authenticateJWT, requireSellerRole, upload.array('images', 10), async (req, res) => {
  ...
  try {
    ...
    await logActivity(req.user.id, 'product_created', 'item', itemId, `Product "${name}" added with SKU: ${sku}`, req);
...
router.put('/seller/products/:id', authenticateJWT, requireSellerRole, upload.array('images', 10), async (req, res) => {
  ...
  try {
    ...
    await logActivity(req.user.id, 'product_updated', 'item', req.params.id, `Product "${name}" updated with SKU: ${sku}`, req);
...
// PATCH /seller/products/:id/status - update product status (active/inactive)
router.patch('/seller/products/:id/status', authenticateJWT, requireSellerRole, async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      'UPDATE item SET status = ? WHERE item_id = ? AND seller_id = ?',
...
router.get('/seller/products/:id', authenticateJWT, requireSellerRole, async (req, res) => {
  try {
    ...
    // Parse image JSON or comma-separated string
    try {
      product.image = JSON.parse(product.image);
      if (!Array.isArray(product.image)) throw new Error();
    } catch {
...
// ---Code para sa Term Test Lab: Email notifications when updating transaction (5pts)----
